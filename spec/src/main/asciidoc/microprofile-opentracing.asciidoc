...
The operation name can be changed via MicroProfile config configuration property `mp.opentracing.server-operation-name-provider`.
The implementation has to provide `path` based operation name. In this case the span operation name
consists of `@Path` values annotations.
...
Configuration of the skip pattern leverages MicroProfile Config specification.
The skip pattern is specified as a string with key `mp.opentracing.skip-pattern` which has to be
compliant with `java.util.regex.Pattern`. An example skip pattern might be `mp.opentracing.skip-pattern=/health|/metrics.*`
